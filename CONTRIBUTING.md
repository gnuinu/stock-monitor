...
```
feat: 모의투자 매수/매도 및 손익 추적 추가
fix: Yahoo 일봉의 null 캔들을 건너뛰도록 수정
docs: README 데이터 소스 섹션 갱신
refactor: MarketDataProvider 추상화 도입
test: 초과 매도·예수금 부족 케이스 검증 추가
build: Gradle Wrapper 추가
...
- 영역이 다른 변경은 **나눠서 커밋**하면 히스토리가 깔끔합니다.
  ```bash
  git add backend/  && git commit -m "feat: ..."
  git add frontend/ && git commit -m "feat: ..."
  ```
